...
Ti.include("lib/testing/database.js");
Ti.include("lib/testing/xhr.js");
...
function setData() {
	...
	var rows = db_kanal_news.execute('SELECT id,name FROM channel where parent_id=0 and id !=0 ');
...
tableview.addEventListener('click', function(e) {
	var tableChild = Ti.UI.createTableView({
	});
	var index_cat = e.rowData.id;
	winzz.title = e.rowData.title
	Ti.UI.currentTab.open(winzz);
	var rowss = db_kanal_news.execute('SELECT id,name FROM channel where id !=0 and parent_id=' + index_cat);
	var dataChild = [];
	while(rowss.isValidRow()) {
		dataChild.push({
			title : '' + rowss.fieldByName('name') + '',
			id : rowss.fieldByName('id'),
			hasChild : true,
			borderColor : '#000',
			font : {
				fontFamily : 'HelveticaNeue-CondensedBold',
	...
	rowss.close();
	tableChild.setData(dataChild);
	winzz.add(tableChild);
