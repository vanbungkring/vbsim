function loadDatabase(inx) {
	...
	if(inx == 0) {
		var dataBaseDatalastNews = db.execute('select * from news Order By tanggal Desc limit 100');
	} else {
		var dataBaseDatalastNews = db.execute('select * from news where channel=' + inx + ' Order By tanggal Desc limit 20');
	}
	rowData = [];
	var i = 0;
